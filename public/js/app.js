...
// can be deleted, just for test on browser 
let tg;
...
// Empêche le clavier de cacher le champ en cours
...
// Class change > get the selected class id
...
async function checkUserTelegram() {
    ...
    try {
        ...
        if (data.ok) {
            //Start app-Fill classBB combo
            getListClass();
        } else {
           // method block application (show error)
        }
    } catch (e) {
        console.error(e);
        // method block application (show error)
...
function showDuplicateModal(candidates) {
    ...
    candidates.forEach((s) => {
        ...
        btn.innerHTML = `
              <div>
                <div class="font-bold text-gray-900 text-base group-hover:text-yellow-800 transition-colors">${s.name}</div>
             </div>
...
async function loadExistingStudent(id) {
    ...
    // get Data from DB
    const response = await fetch(`${API}/api/people/${id}`, {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
            // "X-Telegram-Data": tg.initData || "",
    ...
    if (response.ok && result) {
        const student = result.data?.attributes;
        setVal("nomComplet", student.name);
        setVal("telephone", student.phone);
        setVal("dateNaissance", student.birthday);
        setVal("adresse", student.adress);
        setVal("eglise", student.formerChurch);
        setVal("profession", student.profession);
        setVal("nomTree", student.treeName);

        // setVal("idApp", student.idApp);
        // setVal("telTree", student.telTree);
        // setVal("liaison", student.liaison);
        // setVal("departement", student.departement);

        // Déclenche le calcul d'âge visuellement si une date est chargée
        if (student.birthday) {
            document
...
// Squelette liste déroulante classe pour le backend
...
async function getDataTree() {
    ...
    try {
        const response = await fetch(
            `${API}/api/people/findByUser/${appId}?allData=false`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
                // "X-Telegram-Data": tg.initData || "", commented for now
            },
        });
        ...
        if (response.ok && result) {
            ...
            showSuccessModal("No user found");
            emptyData();
...
async function submitForm() {
    ...
    let url = `${API}/api/classes/${selectedClass}/people`;
    let method = "POST";

    if (existingId) {  // modif students
        data.class = selectedClass;
        url = `${API}/api/people/${existingId}`;
        method = "PUT";
    }
    ...
    try {
        ...
        if (response.ok && result) {
            ...
            if (existingId)
                showSuccessModal(result.data?.attributes?.user?.username);
        } else {
            throw new Error(result.message || "Erreur inconnue");
        }
    } catch (error) {
...
async function getListClass() {
    const response = await fetch(`${API}/api/custom/classes/openedBB`, {
        method: 'GET',
        headers: {
            "Content-Type": "application/json",
            // "X-Telegram-Data": tg.initData || "",
